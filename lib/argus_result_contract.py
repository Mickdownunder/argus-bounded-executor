...
ATLAS_OVERALL_VALUES = {"", "PASS", "INCONCLUSIVE", "FAIL", "SKIPPED"}
RECOMMENDATION_VALUES = {"stop", "new_test", "more_runs", "candidate_for_promotion"}
FAILURE_CLASSES = {
    "",
    "contract_failure",
    "execution_failure",
...
REASON_CODES = {
    "",
    "terminal_success",
    "invalid_operator_routing",
    "argus_step_failed",
    "atlas_delegation_failed",
...
def validate_result(payload: dict[str, object]) -> dict[str, object]:
    ...
    normalized: dict[str, object] = {
        ...
        "atlas_overall": _norm(data.get("atlas_overall")),
        "atlas_recommendation": _norm(data.get("atlas_recommendation")),
        "atlas_run_dir": _norm(data.get("atlas_run_dir")),
        "atlas_summary_file": _norm(data.get("atlas_summary_file")),
    ...
    if normalized["atlas_overall"] not in ATLAS_OVERALL_VALUES:
        raise ValueError("unsupported atlas_overall")
